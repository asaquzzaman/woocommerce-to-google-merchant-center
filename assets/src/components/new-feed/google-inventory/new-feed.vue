<template>
	<div class="woogool-new-feed-warp">
		...
		<form v-if="!loading" class="woogool-new-feed-form" action="" @submit.prevent="submit()" method="post">
			...
			<google-inventory v-show="stage.step == 'second'" :extAttr="extAttr" :gAttrs="gAttrs"  :stage="stage"></google-inventory>
...
<script>
	...
	import FormHeader from '@components/new-feed/google-inventory/form-header.vue'
	import FormContent from '@components/new-feed/google-inventory/form-google-inventory.vue'
	import FormLogic from '@components/new-feed/google-inventory/form-logic.vue'
	...
	export default {
		...
		data () {
			return {
				...
				header: {
					...
					country: {},
					channel: {
						label: 'Google Local Products Inventory',
						id: 'google_inventory'
					},
				},
				gAttrs: [],
		...
		watch: {
			'$route' (route, old) { 
                if(old.name === 'edit_google_inventory') {
                	this.feed_id = false;
